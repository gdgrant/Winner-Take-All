...
def update_dependencies():
    ...
    if par['weight_distribution'] == 'gamma':
        par['W_in_init'] = conn*np.float32(np.random.gamma(shape=0.1, scale=1.0, size = [par['n_input'], par['n_hidden']]))
    elif par['weight_distribution'] == 'uniform':
        par['W_in_init'] = conn*np.float32(np.random.uniform(low=0., high=0.5, size=[par['n_input'], par['n_hidden']]))

    par['W_out_init'] = np.float32(np.random.gamma(shape=0.1, scale=1.0, size = [par['n_hidden'], par['n_output']]))

    if par['EI']:
        if par['weight_distribution'] == 'gamma':
            par['W_rnn_init'] = np.float32(np.random.gamma(shape=0.1, scale=1.0, size = [par['n_hidden'], par['n_hidden']]))
        elif par['weight_distribution'] == 'uniform':
            par['W_rnn_init'] = np.float32(np.random.uniform(low=0., high=0.5, size=[par['n_hidden'], par['n_hidden']]))
