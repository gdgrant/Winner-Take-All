...
class MultiStimulus:
    ...
    def get_tasks(self):
        ...
        elif par['task'] == 'expanded_go':
            self.task_types = [
                [self.task_go, 'go', 0],
                [self.task_go, 'rt_go', 0],
                [self.task_go, 'dly_go', 0],

                [self.task_go, 'go', np.pi/2],
                [self.task_go, 'rt_go', np.pi/2],
                [self.task_go, 'dly_go', np.pi/2],

                [self.task_go, 'go', np.pi],
                [self.task_go, 'rt_go', np.pi],
                [self.task_go, 'dly_go', np.pi],

                [self.task_go, 'go', 3*np.pi/2],
                [self.task_go, 'rt_go', 3*np.pi/2],
                [self.task_go, 'dly_go', 3*np.pi/2],
        ...
        elif par['task'] == 'go_antigo':
            self.task_types = [
                [self.task_go, 'go', 0],
                [self.task_go, 'go', np.pi]
            ]

        elif par['task'] == 'go_dly_go':
            self.task_types = [
                [self.task_go, 'go', 0],
                [self.task_go, 'dly_go', 0]
            ]

        elif par['task'] == 'dms_dmc':
            self.task_types = [
                [self.task_matching, 'dms'],
                [self.task_matching, 'dmc']
            ]

        elif par['task'] == 'dms_dnms':
            self.task_types = [
                [self.task_matching, 'dms'],
                [self.task_matching, 'dnms']
            ]

        elif par['task'] == 'problem':
            self.task_types = [
                [self.task_matching, 'dmc']
            ]
    ...
    def task_dm(self, variant='dm1'):
        ...
        for b in range(par['batch_size']):
            fixation[:int(stim_off[b]),b,:] = par['tuning_height']
            resp_fix[:int(stim_off[b]),b,:] = 1
    ...
    def task_dm_dly(self, variant='dm1'):
        ...
        for b in range(par['batch_size']):
            fixation[:np.int16(resp_time[0,b]),b,:] = par['tuning_height']
            resp_fix[:np.int16(resp_time[0,b]),b,:] = 1
    ...
    def task_matching(self, variant='dms'):
        ...
        if variant in ['dms', 'dnms']:
            ...
            stim1_cat = np.logical_and(np.less(-1e-9, stim1), np.less(stim1, np.pi))
            stim2_cat = np.logical_and(np.less(-1e-9, stim2), np.less(stim2, np.pi))
        ...
        if variant in ['dms', 'dmc']:
            resp = np.where(match, 0, 4) #stim1_int, -1)
        elif variant in ['dnms', 'dnmc']:
            resp = np.where(match, 4, 0) #-1, stim2_int)
        else:
            raise Exception('Bad variant.')
        ...
        stim2_off = -np.ones_like(stim2_on) #stim2_on + 300//par['dt']
        resp_time = stim2_on #stim2_off
        resp_fix  = np.copy(fixation[:,:,0:1])

        for b in range(par['batch_size']):
            fixation[:np.int16(resp_time[b]),b,:] = par['tuning_height']
            ...
            if not resp[b] == -1:
                response[resp_time[b]:,b,np.int16(resp[b])] = 1
